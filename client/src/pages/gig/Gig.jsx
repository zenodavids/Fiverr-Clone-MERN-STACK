import React from 'react';
import './Gig.scss'; // Import the component's CSS styles
import { Slider } from 'infinite-react-carousel/lib'; // Import Slider component from infinite-react-carousel library
import { Link, useParams } from 'react-router-dom'; // Import Link and useParams from react-router-dom
import { useQuery } from '@tanstack/react-query'; // Import useQuery hook from react-query
import newRequest from '../../utils/newRequest'; // Import a utility for making API requests
import Reviews from '../../components/reviews/Reviews'; // Import the Reviews component

function Gig() {
  // Get the 'id' parameter from the URL
  const { id } = useParams();

  // Fetch gig data using useQuery
  const { isLoading, error, data } = useQuery({
    queryKey: ['gig'],
    queryFn: () =>
      newRequest.get(`/gigs/single/${id}`).then((res) => {
        return res.data;
      }),
  });

  // Extract userId from the fetched gig data
  const userId = data?.userId;

  // Fetch user data using useQuery with userId as the query key
  const {
    isLoading: isLoadingUser,
    error: errorUser,
    data: dataUser,
  } = useQuery({
    queryKey: ['user'],
    queryFn: () =>
      newRequest.get(`/users/${userId}`).then((res) => {
        return res.data;
      }),
    enabled: !!userId, // Enable the query only if userId is available
  });

  // Render the gig details and user information
  return (
    <div className='gig'>
      {isLoading ? (
        'loading'
      ) : error ? (
        'Something went wrong!'
      ) : (
        <div className='container'>
          <div className='left'>
            {/* Display breadcrumbs, gig title, and user information */}
            <span className='breadcrumbs'>
              Fiverr {'>'} Graphics & Design {'>'}
            </span>
            <h1>{data.title}</h1>
            {isLoadingUser ? (
              'loading'
            ) : errorUser ? (
              'Something went wrong!'
            ) : (
              <div className='user'>
                <img
                  className='pp'
                  src={dataUser.img || '/img/noavatar.jpg'}
                  alt=''
                />
                <span>{dataUser.username}</span>
                {/* Display star rating based on average */}
                {!isNaN(data.totalStars / data.starNumber) && (
                  <div className='stars'>
                    {Array(Math.round(data.totalStars / data.starNumber))
                      .fill()
                      .map((item, i) => (
                        <img src='/img/star.png' alt='' key={i} />
                      ))}
                    <span>{Math.round(data.totalStars / data.starNumber)}</span>
                  </div>
                )}
              </div>
            )}

            {/* Display gig images using Slider */}
            <Slider slidesToShow={1} arrowsScroll={1} className='slider'>
              {data.images.map((img) => (
                <img key={img} src={img} alt='' />
              ))}
            </Slider>

            <h2>About This Gig</h2>
            <p>{data.desc}</p>

            {/* Display seller information */}
            {isLoadingUser ? (
              'loading'
            ) : errorUser ? (
              'Something went wrong!'
            ) : (
              <div className='seller'>
                <h2>About The Seller</h2>
                <div className='user'>
                  <img src={dataUser.img || '/img/noavatar.jpg'} alt='' />
                  <div className='info'>
                    <span>{dataUser.username}</span>
                    {!isNaN(data.totalStars / data.starNumber) && (
                      <div className='stars'>
                        {Array(Math.round(data.totalStars / data.starNumber))
                          .fill()
                          .map((item, i) => (
                            <img src='/img/star.png' alt='' key={i} />
                          ))}
                        <span>
                          {Math.round(data.totalStars / data.starNumber)}
                        </span>
                      </div>
                    )}
                    <button>Contact Me</button>
                  </div>
                </div>
                {/* Display user information */}
                <div className='box'>
                  <div className='items'>
                    {/* Display user details */}
                    <div className='item'>
                      <span className='title'>From</span>
                      <span className='desc'>{dataUser.country}</span>
                    </div>
                    <div className='item'>
                      <span className='title'>Member since</span>
                      <span className='desc'>Aug 2022</span>
                    </div>
                    <div className='item'>
                      <span className='title'>Avg. response time</span>
                      <span className='desc'>4 hours</span>
                    </div>
                    <div className='item'>
                      <span className='title'>Last delivery</span>
                      <span className='desc'>1 day</span>
                    </div>
                    <div className='item'>
                      <span className='title'>Languages</span>
                      <span className='desc'>English</span>
                    </div>
                  </div>
                  <hr />
                  <p>{dataUser.desc}</p>
                </div>
              </div>
            )}

            {/* Render the Reviews component */}
            <Reviews gigId={id} />
          </div>

          <div className='right'>
            <div className='price'>
              <h3>{data.shortTitle}</h3>
              <h2>$ {data.price}</h2>
            </div>
            <p>{data.shortDesc}</p>

            {/* Display gig details */}
            <div className='details'>
              <div className='item'>
                <img src='/img/clock.png' alt='' />
                <span>{data.deliveryDate} Days Delivery</span>
              </div>
              <div className='item'>
                <img src='/img/recycle.png' alt='' />
                <span>{data.revisionNumber} Revisions</span>
              </div>
            </div>

            {/* Display gig features */}
            <div className='features'>
              {data.features.map((feature) => (
                <div className='item' key={feature}>
                  <img src='/img/greencheck.png' alt='' />
                  <span>{feature}</span>
                </div>
              ))}
            </div>

            {/* Link to payment page */}
            <Link to={`/pay/${id}`}>
              <button>Continue</button>
            </Link>
          </div>
        </div>
      )}
    </div>
  );
}

export default Gig;
